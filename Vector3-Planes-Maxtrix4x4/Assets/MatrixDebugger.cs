...
public class MatrixDebugger : EditorWindow
{
	...
	private void OnGUI()
	{
		...
		if (target != null)
		{
			...
			DrawMatrix(MyMatrix4x4.Translate(target.transform.position));
			EditorGUILayout.Space();
			EditorGUILayout.LabelField("Rotation matrix", style);
			DrawMatrix(MyMatrix4x4.Rotate(target.transform.rotation));
			EditorGUILayout.Space();
			EditorGUILayout.LabelField("Scale matrix", style);
			DrawMatrix(MyMatrix4x4.Scale(target.transform.localScale));
			EditorGUILayout.Space();
			EditorGUILayout.LabelField("TRS matrix",style);
			DrawMatrix(MyMatrix4x4.TRS(target.transform.position, target.transform.rotation, target.transform.localScale));
		}
	}
	...
	private void DrawMatrix(MyMatrix4x4 matrix)
	{
		GUIStyle style = new GUIStyle();
		style.fontSize = 20;
		EditorGUILayout.LabelField("| " + matrix.m00.ToString("00.00") + " " + matrix.m01.ToString("00.00") + " " + matrix.m02.ToString("00.00") + " " + matrix.m03.ToString("00.00") + " |", style);
